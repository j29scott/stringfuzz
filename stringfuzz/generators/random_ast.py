...
def make_random_expression(variables, sort, depth, valid_o = False):
    global _semantically_valid

    # if semantics are going to hell, then randomly reinvent the sort
    if _semantically_valid is False and valid_o is False:
    ...
    random_args = [make_random_expression(variables, arg_sort, shrunken_depth, valid_o) for arg_sort in signature]
